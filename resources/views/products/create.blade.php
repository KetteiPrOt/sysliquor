<x-app-layout>
    ...
    <x-form-background>
        <form action="{{route('products.store')}}" method="POST" class="flex flex-col items-start">
            @csrf
            ...
            <div>
                ...
                >
                    @foreach($types as $type)
                        <option value="{{$type->name}}" @selected(old('type') == $type->name)>
                            {{$type->name}}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mt-2">
                ...
                >
                    @foreach($presentations as $presentation)
                        <option value="{{$presentation->content}}" @selected(old('presentation') == $presentation->content)>
